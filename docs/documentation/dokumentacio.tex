...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{images/point_generation_output.png}
\caption{A generált ponthalmaz vizualizációja}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.6\textwidth]{images/main_output.png}
\caption{A program futásának eredménye: az illesztett kör}
\end{figure}
...
\item \texttt{--known-optimum}: Ismert optimumú tesztesetek validálása.
\item \texttt{--iteration-runtime}: Futási idő és iterációszám kapcsolata.
\item \texttt{--parameter-variation}: Különböző paraméter-beállítások vizuális bemutatása.
\end{itemize}

\subsubsection{Ismert optimumú tesztek}
...
\begin{itemize}
\item \textbf{Szabályos sokszögek:} Háromszög, négyzet, hatszög, tízszög -- a körülírt kör sugara egyenlő a csúcsok középponttól mért távolságával.
\item \textbf{Ellipszisek:} Az optimális befoglaló kör sugara a nagyobb féltengely hosszával egyenlő.
\end{itemize}
Ezek a tesztek lehetővé teszik az algoritmus pontosságának objektív mérését.

\subsubsection{Paraméter variációs tesztek}
...
\subsection{Validáció ismert optimumú teszteseteken}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/known_optimum_validation.png}
\caption{Ismert optimumú tesztesetek: a zöld szaggatott vonal az elméleti optimális kört, a piros vonal a talált megoldást jelöli}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{images/known_optimum_errors.png}
\caption{Az algoritmus hibája az elméleti optimumhoz képest különböző teszteseteken}
\end{figure}

\textbf{Elemzés:}
\begin{itemize}
\item \textbf{Szabályos sokszögek:} A háromszög, hatszög és tízszög esetén az algoritmus 1\% alatti hibával találja meg az optimumot. Ez igazolja, hogy a fitnesz-függvény helyesen vezeti a keresést.
\item \textbf{Négyzet:} A négyzetnél kissé nagyobb (kb. 2\%) a hiba, ami a sarkokon lévő pontok speciális elhelyezkedéséből adódik.
\item \textbf{Ellipszisek:} Az ellipszis alakú ponthalmazok nehezebb feladatot jelentenek, mivel a befoglaló kör középpontja nem esik egybe az ellipszis középpontjával. Ennek ellenére az algoritmus itt is 2-3\% körüli hibával dolgozik.
\end{itemize}
...
\subsection{Futási idő és iterációszám kapcsolata}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/iteration_runtime_analysis.png}
\caption{Futási idő a generációszám függvényében (bal) és az egy generációra jutó idő (jobb)}
\end{figure}

\textbf{Elemzés:}
\begin{itemize}
\item \textbf{Lineáris kapcsolat:} A bal oldali grafikon mutatja, hogy a futási idő lineárisan függ a generációszámtól. A lineáris illesztés meredeksége körülbelül \textbf{2.5 ms/generáció}.
\item \textbf{Konstans iterációnkénti idő:} A jobb oldali grafikon igazolja, hogy az egy generációra jutó idő állandó, függetlenül az összes generáció számától. Ez azt jelenti, hogy nincs rejtett többletköltség a hosszabb futtatásoknál.
\item \textbf{Tervezhetőség:} Az ismert meredekség alapján a futási idő jól becsülhető: $T \approx 2.5 \cdot G$ milliszekundum, ahol $G$ a generációk száma.
\end{itemize}

\subsection{Paraméter variációk hatása konkrét példákon}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{images/parameter_variations.png}
\caption{Különböző nehézségű ponthalmazok és a megtalált befoglaló körök}
\end{figure}

\textbf{Elemzés:}
\begin{itemize}
\item \textbf{Ideális eset (zaj=2, alakhiba=0, outlier=0):} Az algoritmus szinte tökéletesen illeszti a kört a tiszta adatokra.
\item \textbf{Közepes zaj (zaj=5, alakhiba=0.1, outlier=5):} Enyhe romlás, de a kör még mindig jól illeszkedik.
\item \textbf{Nagy zaj (zaj=15):} A nagyobb szórás miatt a körnek nagyobbnak kell lennie.
\item \textbf{Nagy alakhiba (alakhiba=0.3):} Az ellipszis-szerű torzítás miatt a befoglaló kör jelentősen megnő.
\item \textbf{Sok outlier (20 db):} A távoli pontok miatt a körnek az összes pontot le kell fednie.
\item \textbf{Extrém eset:} A kombinált nehézségek esetén a legnagyobb a kör, de az algoritmus még mindig konvergál.
\end{itemize}

Ezek a példák szemléletesen mutatják, hogy az algoritmus robusztus a különböző hibatípusokkal szemben, és minden esetben matematikailag helyes megoldást ad.
...
\item  \textbf{Pontos:} Megfelelő paraméterezés (0.1 körüli mutációs ráta) mellett stabilan megtalálja a szuboptimális vagy optimális megoldást. Ismert optimumú teszteseteken 1-3\% hibával dolgozik.
...
\item  \textbf{Kiszámítható:} A futási idő lineárisan függ a generációszámtól, így jól tervezhető.
\end{enumerate}
